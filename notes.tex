...
\textbf{Attributes}:
\begin{itemize}
\item Should be simple and single-valued (atomic data)
\item Should document default values, constraints, synonyms, and aliases
\item Derived attributes should be clearly identified and include source(s)
\item Should not be redundant unless this is required for transaction accuracy, performance, or maintaining a history
\item Nonkey attributes must be fully dependent on the PK attribute
\end{itemize}
\textbf{Relationships}:
\begin{itemize}
\item Should clearly identify relationship participants
\item Should clearly define participation, connectivity, and document cardinality
\end{itemize}
\textbf{ER Model}:
\begin{itemize}
\item Should be validated against expected processes: inserts, updates, and deletions
\item Should evaluate where, when, and how to maintain a history
\item Should not contain redundant relationships except as required (see attributes)
\item Should minimize data redundancy to ensure single-place updates
\item Should conform to the minimal data rule: All that is needed is there, and all that is there is needed
\end{itemize}
\end{document}